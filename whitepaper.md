...
**Authors:** Jeremy Johnson & Javier Varona Zavatti, Co-Founders, Dankest, LLC · **Version 1.4**
...
- **DISPENSER** is a vending machine: send `GET_AMOUNT` of a coin/token to the address and automatically receive the dispensed token (the synthetic `DISPENSE` action records the payout). Create (v0), cancel (v1), edit (v2). Supports fiat-denominated pricing via the oracle. Third-party dispensers require the target address to opt in. Up to 1,000 dispenses per fill, with up to 5 refills (6,000 lifetime); 1-hour close window. A dispenser may *give* ownership (`GIVE_OWNERSHIP`) but cannot request it.
...
- **ROLLCALL** is a validator-broadcast, DOGE-only presence proof for one roll-call epoch: each present validator signs a canonical bound to the BTC epoch block's ledger hash, so a signature cannot be produced before that block exists and proves the key was live in that epoch. The BTC indexer closes each epoch by proving the DOGE action, records who was absent, and evicts a validator absent for enough consecutive epochs. Eviction is a deactivation, never a burn: the stake refunds after the ordinary cooldown and the validator may re-enter with a fresh STAKE (§11.1). *(pre-launch: armed on testnet; mainnet ships inert, its activation height operator-owned.)*
...
| Controller guard ceiling (per guard run) | 200,000 gas, reserved up front on BTC, charged on allow only |
