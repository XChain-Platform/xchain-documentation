...
- **Node.js**: 22 (22.x LTS) exactly, required to run the xchain-node CLI. Node 18 and earlier fail on the ESM-only `mariadb` driver (`ERR_REQUIRE_ESM`); Node 24 fails the Node ABI 127 consensus-runtime pin in the `xchain-vm` the indexer and explorer pull in.
