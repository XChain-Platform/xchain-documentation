...
| **encoder** | Stateless; turns an ACTION string + UTXOs + pubkey into an unsigned PSBT | None |
...
| **explorer** | Stateless REST + JSON-RPC + WebSocket + web UI over the indexer DB | None |
| **hub** | Decentralized config oracle, price oracle, cross-chain coordinator, attestation engine, governance | MariaDB (hub DB) |
| **sync** | Replicates decoder + indexer DBs to validators via REST snapshots + WebSocket streaming | None |
...
- **v0 (request, VM-emitted only):** `request_id` is the SHA-256 of `tx_hash ‖ root_action_index ‖ emitter_path ‖ contract_index ‖ emitter_position` (colon-delimited; `emitter_path` is the `>`-joined per-execution call path, empty for a root on-chain action; the indexer re-derives it to defend against a compromised VM). Carries provider id, payload, callback method/params, redundancy, and deadline. A user-broadcast v0 is rejected.
