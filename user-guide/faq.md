...
The exceptions belong to the token's issuer rather than to the platform, and there is more than one of them. Each is a per-token property the issuer chooses, and each is readable on-chain before you acquire the token:

- **Callback (force-recall).** If a token was created with a callback configured, and its issuer has not locked that setting off, the issuer can recall that specific token from every holder: each holder's balance goes back to the issuer and each holder is credited the disclosed settlement amount in the settlement token, with no holder signature involved. It does not exist on a token issued without a callback. See [Can a token be permanently destroyed?](#can-a-token-be-permanently-destroyed) below.
- **A controller binding.** The token's owner can bind the token to a contract that gets the final say over sends, trades, mints, burns, staking, or ownership transfers. Its guard runs before your action settles and can deny it, so a binding on transfers (or on the catch-all class) can stop you moving a balance you already hold, and a binding on trades can route part of your sale proceeds to addresses the contract names. Unlike the settings above, a binding is not covered by any lock flag and can be added after the token was issued, so it is worth re-checking rather than checking once. See [Controller-Bound Tokens](../protocol/controller-bound-tokens.md).
- **Allow lists and block lists.** These gate the sending address as well as the receiving one, so taking a holder off the allow list, or adding them to the block list, freezes the balance they already hold until the lists change. See [Access Control](./creating-tokens.md#access-control).

None of these hand an override to XChain, to its validators, or to any company. They are token-level policies that issuers publish on-chain, and a token that uses none of them has none of them. Read a token's settings and its controller bindings before you acquire it.
