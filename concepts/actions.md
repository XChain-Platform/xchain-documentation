...
| `ROLLCALL` | Validator liveness roll call, published on **Dogecoin**. Carries a set of Ed25519 signatures over a canonical bound to a Bitcoin epoch block's `ledger_hash`, so a signature proves the key was operating inside that epoch and cannot be pre-signed. Any number of roll calls may land per epoch from anyone, and the present set is their union, so no publisher holds the absence list. The BTC indexer closes each epoch, records absences, and evicts a source absent for K consecutive rolled epochs by deactivating its stake (refunded after cooldown, never burned). |
