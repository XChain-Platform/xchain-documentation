...
| `EXPLORER_RATE_LIMIT_RPM` | _(unset; explorer defaults to `500`)_ | Passed through to the explorer: requests per minute per IP across its whole API. A private venue reached through one tunnel or proxy is a SINGLE IP to this limiter, so every browser and every automated run on that host shares one budget; a browser-driven test suite alone sustains several hundred a minute. Left unset, the explorer's public-facing default applies. |
