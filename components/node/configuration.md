...
| `XCHAIN_NODE_NO_SELF_UPDATE` | Set to `1`, `true`, or `yes` to make `update` leave the CLI checkout where it is and update only the services (default: unset, so an `update` that targets a release first moves the CLI to that release, verifies the release tag's signature, reinstalls its dependencies and re-runs the command on the new code). For a checkout you manage yourself. |
| `XCHAIN_NODE_UPDATE_TARGET` | Set by the CLI on the command it re-executes after moving itself to a release (the value is that release tag, e.g. `v0.16.0`); the re-run reads it as the resolved target and as the guard against moving again. Not meant to be set by hand; default unset. |
...
| `XCHAIN_NODE_STAKE_WIF` | **Credential.** Private key for the stake/fee wallet, read by `validator init --import-stake-key` so an existing funded key can be adopted instead of generating a new one. Without it that flag falls back to a hidden prompt. Supplying it non-interactively is the point: a scripted install should not need a TTY. |
...
| `XCHAIN_NODE_SKIP_NODE_TIP_GUARD` | Set to `1`/`true`/`yes` to skip the restore-time comparison between a bootstrap archive's end height and the coin node's tip. Default unset: a node below the archive is reported as `WAITING FOR NODE` when the service waits it out, and the restore is refused for a service image that would read the lower tip as a reorg. Logs a loud warning when skipped. |
