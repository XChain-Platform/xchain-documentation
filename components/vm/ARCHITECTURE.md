...
| `gateway-emit.js` | Emit API builder: 19 action types (SEND through MESSAGE and VOTE, plus `execute` for cross-contract calls and `crossExecute` for cross-chain calls), parameter validation, gas charging |
...
| `validator.js` | ActionValidator: pre-validates emitted actions against the 21 allowed action types (`SEND`, `DESTROY`, `ISSUE`, `MINT`, `ORDER`, `DISPENSER`, `DIVIDEND`, `AIRDROP`, `CALLBACK`, `FILE`, `LIST`, `COINPAY`, `SWEEP`, `LINK`, `BROADCAST`, `MESSAGE`, `ATTEST`, `SLASH`, `EXECUTE`, `XCALL`, `VOTE`) and checks params shape |
| `lint-core.js` | Dependency-light (no isolated-vm) acorn-only consensus validation: acorn metering pass, reserved identifier check, banned transcendental Math.* (`sqrt`/`pow`/`log`/`log2`/`log10`), banned literals (BigInt, RegExp), banned async surface (`async`/`await`/`Promise`), and float warnings. Holds `CONSENSUS_RULES` (the set of deploy-blocking rule names) and `lintSource()`. Vendored byte-for-byte into `xchain-sdk/src/contract/lint-core.js`; a CI SHA-256 parity guard fails on drift. |
| `syntax.js` | Deploy-time validation: V8 syntax check (throwaway isolate, the one step requiring isolated-vm), then delegates all acorn-coverable consensus rules to `lint-core.lintSource()`. Re-exports `findBannedMathCalls`, `findBannedLiterals`, and `findBannedAsync` for backward compatibility. The `banned-async` rule is consensus-gated: the indexer passes a flag derived from the `VM_BANNED_ASYNC` flag-day so a from-genesis replay reproduces the historical deploy verdict. |
