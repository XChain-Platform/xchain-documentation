...
| `HUB_AUTH_RATE_LIMIT_RPM` | No | `60000` | Requests allowed per IP per 60-second window on the JSON-RPC surface for callers whose `x-api-key` header matches a key this hub has configured, metered in their own bucket so the fleet's own indexers and replay traffic keep a high budget while keyless callers stay at `HUB_RATE_LIMIT_RPM`. A wrong key lands in the public bucket, and a keyless hub has no authenticated bucket, so every caller is public there. A request is charged to one bucket only, and a batch is charged one token per call. The mirror-bootstrap routes `/hub-db/snapshot/*` stay in the `HUB_SNAPSHOT_RATE_LIMIT_RPM` bucket, and the `HUB_RATE_LIMIT_EXEMPT_LOCAL` exemption applies here too. An unset, unparseable or non-positive value keeps the default, raised to `HUB_RATE_LIMIT_RPM` if that is higher. |
...
| `PBFT_SNAPSHOT_TOLERANCE_BLOCKS` | No | `144` | How far a config-PBFT `PRE_PREPARE`'s leader-stamped `btcBlockHeight` may deviate from this hub's own BTC tip before a federated follower declines to PREPARE. The stamped height selects the validator set, the leader and the stake-weighted-quorum outcome, so an unbounded one lets the proposer choose all three; a follower that cannot resolve a tip of its own declines. Single-node hubs are unaffected. |
| `ORACLE_SNAPSHOT_TOLERANCE_BLOCKS` | No | `144` | The same bound on the price-round `PROPOSE` path: how far a leader-supplied `btcBlockHeight` may deviate from this hub's own BTC tip before a federated follower drops the round. |
...
| `XCHAIN_HUB_SKIP_ZERO_CONF_ASSERT` | No | _(unset)_ | Set to `1` to bypass the boot assertion that `ATTEST_ZERO_CONF_ACTIVATION` is at or above both `ATTEST_RESPONSE_MIRROR_ACTIVATION` and `ATTEST_RESPONSIBLE_WIDENING_ACTIVATION` on every network where it is armed. Only for a venue where **every** hub runs the same maps: below the mirror height a round would run at zero confirmations and burn a broadcast fee against a reorged request, and with widening unarmed the flip would remove the wait with no headroom behind it. On `mainnet` and `testnet` a violation otherwise refuses to start (`ZERO_CONF_ORDERING`); standalone and regtest warn instead. The bypass logs a warning every time it is taken. |
...
| `ORACLE_CLAMP_RESEED_MS` | No | `60000` | Milliseconds between re-seeds of the last-finalized clamp reference (default: 1 minute). Cadence only, and deliberately NOT federation-uniform: it decides how promptly a hub catches up to finalized rows it already holds, never what any hub clamps to, so it needs no flag day. A longer interval degrades toward clamping against a stale reference; a shorter one costs one indexed query per pass. |
...
| `ATTESTATION_POLL_MS` | No | `15000` | How often a validator hub polls its indexer for new attestation requests, in milliseconds. |
| `ATTESTATION_ROUND_TIMEOUT_MS` | No | `120000` | How long an attestation round may run before it is abandoned, in milliseconds. A round that times out leaves the request for a later round rather than failing it. **Coupled, so retune it deliberately:** the request-seen window is sized to stay outside a live round, and both the round and the consensus engine read this one value. Raising it without that in mind lets a second window re-open a request whose round is still pending, which pays a provider a second time for the same fetch. |
| `ATTESTATION_FETCH_TIMEOUT` | No | `20000` | Budget in milliseconds for one external provider fetch, and the same budget the `judge_model` strategy gives its judge call, so both legs of a round agree on how long a slow vendor is allowed to be. Raised from `10000` by operator ruling: a healthy-but-slow provider that crossed 10 s was aborted, leaving the round with fewer independent bodies than its redundancy asked for, which `byte_equality` scores as a `no_quorum` rather than as a slow vendor. Keep it well under `ATTESTATION_ROUND_TIMEOUT_MS`, which stays the terminal backstop. A zero, a negative or a non-numeric value warns and falls back to the default. |
| `ATTESTATION_PROPOSER_SEEN_MAX` | No | `10000` | Cap on the in-memory record of which validators have proposed for a given attestation request. This outlives a single round's own state, since a round that times out is torn down while the request itself lives on across retries, so leader rotation still needs to know who already spoke for it. Ring-bounded FIFO eviction, so a flood of distinct request ids cannot grow it without bound. |
...
| `ORACLE_PUBLISH_SELF_CHAIN_MAX_DEPTH` | No | `4` | How many wires one publish pass may chain onto its own change. A catch-up sweep broadcasts several wires within one second, and each spends a confirmed output and leaves its change unconfirmed, so the wires past the address's confirmed-output count would otherwise see only dust and fail to build. Within a single pass a wire may therefore spend the change of a wire that same pass broadcast, which is the one exception to the setting above; the count bounds how far that may chain. Only this pass's own transactions qualify, so a third party's unconfirmed payment stays unspendable, and a later pass still defers entirely when no confirmed output is available. |
...
| `ORACLE_BATCH_CATCHUP_BACKLOG_INTERVAL_MS` | No | `60000` (1 minute) | The catch-up sweep cadence while a backlog exists, used instead of `ORACLE_BATCH_CATCHUP_INTERVAL_MS` whenever more than four closed windows are still buffered. A sweep proposes at most four windows, so at the hourly idle cadence a backlog of several hundred windows could never drain; at a minute the same backlog is walked in a few hours. Capped at `ORACLE_BATCH_CATCHUP_INTERVAL_MS`, so it can only make the sweep faster, never slower. |
| `ORACLE_BATCH_CATCHUP_MAX_ATTEMPTS` | No | `6` | Failed catch-up proposals after which a window is retired from the sweep, provided it is also old enough (see `ORACLE_BATCH_CATCHUP_RETIRE_AFTER_MS`). Retirement is a memo entry, not a deletion: the buffered rounds stay where they are and every other bound behaves as before; the window simply stops consuming a re-proposal slot that the windows behind it need. `0` disables retirement, so every window is re-proposed forever, for an operator who would rather a stuck backlog stay visible than be retired quietly. |
| `ORACLE_BATCH_CATCHUP_RETIRE_AFTER_MS` | No | `21600000` (6 hours) | Minimum age of a window's FIRST failed catch-up attempt before the attempt count may retire it. Both bounds are needed: a count alone would retire a window that lost six attempts in six minutes to one peer rebooting, and an age alone would retire a window that has only been waiting its turn behind a long backlog. Together they describe the one hopeless shape, a window this hub has proposed repeatedly over hours that the federation will not co-sign because its contents diverge. |
...
| `ATTEST_BATCH_PUBLISH_ENABLED` | No | `true` | Set to `false` to stop this hub publishing attestation batches on-chain, halting the outbound DOGE spend during an incident without tearing the pipeline's configuration down. Consensus participation is unaffected. A halted publisher **skips** each window rather than buffering it, so re-enabling does not flood the rail with a backlog. Read from the environment first, then the validator config table under the same key. |
| `ATTEST_BATCH_BUFFER_PATH` | No | `./data/attest-batch-buffer.jsonl` | Durable record of what each attestation batch window was built from at the moment it published, so an operator replaying a dead-lettered or quarantined window has its content instead of reconstructing it from tables that have since moved on. Point at persistent storage. The dead-letter file sits beside it, at the same path with `.deadletter.jsonl` in place of `.jsonl`. Belongs to the batch publisher alone and must not be pointed at the price publisher's buffer. Read from the environment first, then the validator config table under the same key. |
| `ATTEST_BATCH_MAX_REFUSAL_ATTEMPTS` | No | `3` | How many times a window whose HEAD wire was refused **before it could be sent** is rebuilt and re-proposed before it latches like any other broadcast failure. The shapes that qualify prove no transaction left the process: an encoder HTTP 4xx such as insufficient funds or unconfirmed change, or an `ECONNREFUSED`/`ENOTFOUND`/`EAI_AGAIN` that never connected. On those the publish-intent marker is withdrawn and the window is retried with byte-identical content on the next cycle instead of being quarantined for an operator; at the bound it latches once and quarantines as before. A failure on any later wire, and every ambiguous failure, latches immediately and is unaffected by this setting. |
| `ORACLE_BATCH_SIGN_TIMEOUT_MS` | No | `15000` | How long the attestation batch leader waits for a signing quorum on an assembled window. Shares its name with the price rail's setting under Oracle Publishing above, and the two are NOT the same knob in practice: this one is read from the environment first and defaults to `15000`, while the price rail reads the validator config table only and defaults to `60000`. An operator exporting this variable moves this rail alone. |
...
| `ATTEST_RELAY_EVICT_GRACE_BLOCKS` | No | `144` | How long the relay keeps remembering a request it has already relayed, counted in blocks on the chain the request came from, past that request's own deadline and on top of that chain's confirmation depth. Once the origin chain is that far past the deadline the request can never be serviced again, so the relay forgets it and rewrites the queue file without it: this is what stops both the queue file and the in-memory sets from growing for as long as the process runs. Raise it to keep records longer. Lowering it is the risky direction, because forgetting a request the origin chain has not finished expiring yet lets the relay send it a second time and pay a second fee. |
