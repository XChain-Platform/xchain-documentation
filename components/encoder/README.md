...
| `MAX_CPFP_UPLIFT_SAT` | No | `10000000` | Ceiling, in base units, on the extra fee a transaction spending unconfirmed inputs may add so that its whole mempool package reaches the target rate. Miners select by ancestor fee rate, so a transaction behind cheap unconfirmed parents stays unmined however much it pays on its own bytes. `0` turns package-aware sizing off entirely |
| `FEE_NO_ESTIMATE_RELAY_MULTIPLIER` | No | `10` | Multiple of the node's own relay floor charged on a non-mainnet chain when `estimatesmartfee` has no data to answer with. Ten is the documented recommended rate and some quiet chains' miners do not accept it: on Dogecoin testnet, measured inclusion starts around 0.03 DOGE per kB while 0.0102 sits unmined. Which rate a chain accepts is a property of that chain, so measure and raise it where needed (`100` gives 0.1 DOGE per kB). Mainnet is unaffected: a missing estimate there raises an error rather than a guess |
| `FEE_ESTIMATE_SANITY_CEILING` | No | Per chain: `0.01` BTC/kB, `0.01` LTC/kB, `10` DOGE/kB | Absolute per-chain sanity ceiling, in COIN/kB, on the raw `estimatesmartfee` result before anything else sees it, including the caller-facing `MAX_FEE_RATE_KB`/`MAX_FEE_RATE_MULTIPLIER` caps, which derive their own relative ceiling from this anchor rather than bounding it directly. A non-regtest chain whose node returns a feerate above the ceiling is clamped to the ceiling and logged rather than propagated. Overrides the coin-scale default (parsed with `parseFloat`) for every chain at once; a non-positive or unparseable value keeps the per-chain default, and an unrecognized or missing `NETWORK` coin prefix falls back to the bitcoin-scale default |
