...
| `anchor_actions` | Per-action ANCHOR records, keyed `(action_index, section_index)`. Versions: 1=checkpoint+archive, 2=archive continuation, 6=archive+publisher attestation, 7=checkpoint bundle. Archive versions write one row at `section_index` 0; a v7 bundle writes one row per per-chain section, all sharing the bundle's `action_index`. Stores signed state hashes, `match_batch_seq`, `archive_b64` (gzip chunk), and `validator_signatures`. Enables full platform state recovery from a chain parse alone |
