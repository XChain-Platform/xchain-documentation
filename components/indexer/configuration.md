...
| `INDEXER_POLL_SILENT_MS` | How long the poll loop may go without completing an iteration before the container reports unhealthy. One block can hold a single iteration across several sequential barrier waits, so the default doubles the stall grace again to be sure this never fires on a block that is merely slow. Measures loop **liveness**, never chain progress, so unlike the stall signal it has nothing to do with commits. Operational only, **not** a consensus parameter. | `2 × INDEXER_HEALTH_STALL_GRACE_MS` |
...
| `ANCHOR_PROOF_TIMEOUT_MS` | Per-request timeout for the DOGE anchor proof read. A timeout is treated as "cannot tell", which defers the block; it is never read as "not mined". | `15000` |
