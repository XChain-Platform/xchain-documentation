...
|---|---|---|---|
| [**DEPLOY**](../../protocol/actions/deploy.md) | Deploy a JavaScript smart contract to the VM. v0 = standard. v1 = stakeable (adds `COOLDOWN_BLOCKS` + `SLASH_DESTINATION` so the contract can accept STAKE v3 actions). v2/v3 = the assembling half of a chunked deploy (v3 stakeable), carrying only `CODE_HASH`; v4 = a chunk carrier, one ordered slice of the base64 source. Behind the `DEPLOY_DEFERRED_ASSEMBLY` flag day, an assembling DEPLOY that lands before all its carriers goes `pending: CODE_HASH (awaiting chunks)` instead of failing, and whichever action later completes the group deploys the contract at its own index; a second assembler for an already-pending group is `invalid: CODE_HASH (duplicate pending)`. | Syntax validation (V8 + acorn ES2020 + `__gas` check), code size ≤ 64KB, sufficient XCHAIN for gas. Creates derived address `C:<CHAIN>:<action_index>`. Optionally runs constructor. v1 staking fields immutable after deploy; `SLASH_DESTINATION` without `COOLDOWN_BLOCKS` is rejected; `BURN` sentinel resolves to chain burn address. | Yes: `VM_DEPLOY_BASE + (bytes * VM_DEPLOY_PER_BYTE)` + constructor gas |
