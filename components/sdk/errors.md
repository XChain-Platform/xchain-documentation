...
| `BATCH_CONSTRAINT` | `count` (for MINT/ISSUE violations), `limit` (for the command cap) | A BATCH protocol rule was violated (nested BATCH, DEPLOY action, more than 1 FILE, more than 1 MINT of the same `tick`, more than 1 top-level ISSUE, more than 250 commands). Child issuances such as `JDOG.1` do not count against the ISSUE limit |
