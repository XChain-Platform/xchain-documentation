...
| `XCHAIN_REQUIRE_SIBLINGS` | Makes `bin/check-preflight-drift.js` treat an unresolved `xchain-indexer` checkout as a finding instead of a skip, so a CI job whose checkout step was dropped fails loudly rather than reporting success having compared nothing. Only the exact value `1` enables it. No default: when unset the gate exits 0 and says it skipped the sibling checks. Development tooling only. | Drift checker |
