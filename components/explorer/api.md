...
| `indexer_state` | Per-coin reason the indexer trails the decoder: `live` when there is no lag, `future_block_wait` when the next block is dated ahead of this host's clock (no node anywhere could have committed it yet, so the pause is consensus rather than failure), `behind` when that next block is already admissible and still uncommitted (the state worth alerting on), or `null` when it cannot be determined. Read this rather than `tip_future_seconds` to tell a deliberate wait apart from a wedge: `tip_future_seconds` describes the block already committed, which is always past-dated, so it reads `0` for the whole `future_block_wait` duration. Only coins this instance measures appear here |
| `next_block_time` | Per-coin unix timestamp the decoder recorded for the next block the indexer must commit (`last_block` + 1), read from the decoder DB since the indexer has not committed that block yet. `null` when the indexer is already at the decoder tip, the decoder DB is unreachable, or that block carries no usable timestamp. Only coins this instance measures appear here |
| `next_block_future_seconds` | Per-coin seconds the next block to be committed is dated ahead of this host's clock; `0` when it is not ahead, `null` when unknown. This is the honest measure of a consensus wait that `tip_future_seconds` is often mistaken for. Only coins this instance measures appear here |
| `indexer_wait_clears_at` | Per-coin ISO-8601 instant a `future_block_wait` ends: the moment this host's clock reaches the next block's timestamp and the block becomes committable. `null` in every other state. Lets a client show a countdown instead of leaving a confirmed but unindexed transaction looking lost |
...
Validator federation data. `validators` rows are the on-chain active set, and each one also carries the hub's view of the same signing pubkey: `hub_addr` (the validator's network address), `hub_chains` (the chains it serves) and `hub_status`. The status merges two hub sources. The hub's manual registry (`registervalidator`) supplies `active`, `suspended` or `removed` plus the address and chains when it has a row; on a live federation that registry is normally empty, because membership is the on-chain stake set. The gossiped `validator_capabilities` rows supply the rest: `active` when at least one capability is qualified, self-tested and enabled, `peered` when the hub has heard the pubkey over P2P but no capability is active yet (typically a hub that cannot read its own stake: no BTC indexer). `unregistered` means the hub answered and has never heard from the pubkey. The address and chains are `null` for gossip-derived entries. All three are `null` when neither hub source is reachable, which means unknown rather than unregistered. `stakes` and `delegations` `type` values: `block`, `address`, `source`. `rewards` `type` values: `address`, `source`. `delegation_revocations` (DELEGATE v2/v3 signing-key revocations) and `collects` (COLLECT validator reward claims) `type` values: `block`, `address`, `source`. `full_node_verifications` `type` values: `block`, `epoch`, `pubkey`, `address`. Contract-targeted staking (`contract_stakes`, `contract_unstakes`, `contract_delegations`) and `slash_events` support types: `block`, `address`, `contract`.
...
Hub-only federation state read from the co-located hub DB (tables that have no on-chain action). The hub's own validator registry has no endpoint or page of its own: its addr, chains and registration-status columns, merged with the per-pubkey summary of `validator_capabilities`, are folded onto `/{COIN}/api/validators` and the `/{COIN}/validators` page, so the on-chain active set and what the hub knows about each key read as one table. `validator_capabilities` `type` values: `capability`, `pubkey`. `governance_proposals` `type` values: `status`, `parameter`, `proposal`. `governance_votes` `type` values: `proposal`, `voter`. `capability_slash_events` (SLASH wire actions) `type` values: `block`, `capability`, `pubkey`, `address`. `oracle_prices` (user-published PRICE v1 rows, hub-mirrored) `type` values: `token`, `address`.
...
OpenAPI 3.1 specification for all explorer REST endpoints. Regenerated by `docs/openapi.build.js`; kept in sync with the route tables by `test/unit/http/openapi_coverage.test.js`.
