...
⬜ It reports `CLEAN`. `STALE` prints the commits that touched what each screenshot shows, and under each one the files that commit changed inside those paths. The two honest ways out are the same as on the other store pages: recapture at the tag, or record in the release record why none of those commits can change these pixels. `INCONCLUSIVE` means it could not tell at all, which is not a pass.  

⬜ **Judge it on the files, not on the subject line.** A subject says what a commit was for, not how far it reached, and each screenshot depends on whole shared directories rather than a per-file list, so the scan over-reports by design. The files are what make an over-report cheap to dismiss. This lane is where that cuts the other way, too: measured 2026-08-08, `ddc94971` changed three files these screenshots depict, one of them the stylesheet whose fix was the chopped action button a Mac App Store screenshot had already caught, so it was a genuine `STALE` here while the identical verdict on the Chrome lane is a false alarm from one shared file the extension popup renders differently. That one was answered the first way: re-captured 2026-08-13 at `2e29d0f1`, and the new Home image shows the action row wrapping onto a second line instead of the chopped button, which is the fix those three files carried.  
...
captures each view. Every one shows the demo wallet: synthetic balances, an
address that is never funded, in a throwaway profile the script deletes when it
exits.

The capture is reproducible, which is the property that makes the pin below
worth reading. Before each run the script freezes the two inputs the demo lane
would otherwise randomize, the demo wallet's seed (a published test vector, so
the address in these public images is the same every time and no reader mistakes
it for a real one) and the clock every synthesized timestamp is dated against.
Re-capturing an unchanged tree therefore rewrites the same bytes, measured twice
in a row on 2026-08-13. A hash that moves means the PRODUCT moved, not that the
demo rolled different dice.
