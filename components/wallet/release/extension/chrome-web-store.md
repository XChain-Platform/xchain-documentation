...
**"The signature is OK" means the release key made it, and until 2026-08-06 this step could not tell you that.** `verify.sh` ran a bare `gpg --verify`, which answers whether somebody in your keyring signed the manifest and prints `Good signature from` when the answer is yes. This project keeps three GPG keys in its orbit that the [verify-release page](../verify-release.md) states outright are not interchangeable, and the release machine holds all three, so on that machine the two questions had the same green answer. It was found by rehearsing this very phase against the real CI-built extension zip: the manifest was signed with the tag-signing key by mistake and this step reported `ok`.

⬜ The output carries a `signer ok - <fingerprint>` line, and that fingerprint is the release key's. `verify.sh` now refuses a good signature from any other key, naming both fingerprints when it does. Run from a checkout it takes the expected value from `docs/release-key-pin.json`; anywhere else, pass `--key <fingerprint>` from [`https://xchain.io/security`](https://xchain.io/security).  
...
⬜ It reports `CLEAN`. **If it reports `STALE`, it prints the commits that touched what each asset depicts, and under each one the files that commit changed inside those paths**, and there are two honest ways out, neither of which is uploading anyway: rebuild the extension at the tag and re-run `packages/extension/scripts/capture-listing-screenshots.mjs`, which re-pins as it goes; or read the listed commits and record in the release record why none of them can change these pixels. The tool deliberately cannot tell a cosmetic commit from a visible one, so it names them instead of guessing. `INCONCLUSIVE` means it could not tell at all (no pin, or a checkout without the history), which is not a pass.  

**Judge it on the files, not on the subject line, because the subject line will mislead you here.** A commit's subject describes what it was for, not how far it reached, and the assets depend on whole shared directories (`packages/core/src/shared`, `packages/core/src/ui`) rather than a per-file list nobody would keep current. So the scan over-reports by design, and the file list is what makes an over-report cheap to dismiss instead of a reason to re-shoot the listing. Measured 2026-08-08: `ddc94971`, subject `fix(desktop): the wallet's settings screen was dead`, flagged all three Chrome screenshots. Read as a subject it is a desktop change that cannot touch an extension popup; read as a file list it is one shared stylesheet, `packages/core/src/shared/routes/Home.module.css`, which the popup does render. The same commit is a genuine `STALE` on the Mac App Store lane, where it changed three depicted files and the fix is visible. One verdict, two correct answers, and only the files tell them apart.  

**`STALE` has two directions, and at submission time the second one is the likelier.** A `STALE:` line means the images are OLDER than the tag. An `AHEAD:` line means they are NEWER: the capture ran against a build the tag you are uploading does not contain, so the screenshots advertise a product the upload does not have. That is not the exotic case here. **The tag you may cut is the last commit carrying a green CI run, while captures get re-taken on whatever the tip is**, so the images routinely run ahead of the release. Until 2026-08-07 this direction was invisible: the tool compared in one direction only and printed `CLEAN`, measured on the day with the pin five commits ahead of the sole taggable commit. Either re-capture at the tag you are submitting, or submit the ref the images depict; do not upload the pair as they stand.  

⬜ If the run prints a `NOTE:` line, read it. It means the capture is ahead of the tag but no surface any asset depicts moved in between, so the images and the upload still show the same product. That is a pass, and it is printed rather than left silent so the absence of a warning is never mistaken for the tool having checked nothing.  
...
⬜ **The store-version monitor is live.** The script exists (`tools/release/store-version-monitor.mjs`, gated by `test/smoke/audits/store-version-monitor.smoke.js`). What does not exist yet is the running CHROME job, and that is what this box is about. **Identify it by `CWS_MAIN_ITEM_ID`, because since a job running this exact script is already installed and firing on the monitoring host and it is NOT this one:** the Android lane arms the same file with `--no-chrome`, which watches the Play listing and checks no Chrome item at all. A `crontab -l` that shows `store-version-monitor.mjs` scheduled and firing therefore satisfies the sentence below without satisfying this box, and ticking it on that evidence flips the extension public with no rogue-publish monitoring, which is the single thing this box exists to prevent. The Chrome job is the entry carrying `CWS_MAIN_ITEM_ID=<the id Chrome assigned at first upload>`; a line carrying `--no-chrome` is the Play lane. Two things are still missing and both come out of this ceremony: the item ID (Chrome assigns it at first upload, Phase 6) and an operator running the install documented in `tools/release/README.md`. Do not flip to public before it is running: this monitor is what turns a rogue or compromised publish into a same-day alert instead of a silent one, by reading the publish log against the live store version. Confirm its scheduled job is actually installed and has fired at least once before treating this box as checked, not merely that the script exists. **The script itself refuses to be mistaken for a running check:** with no item ID set it exits 2 (config error), never 0, precisely so a job that never really ran cannot read as an all-clear.
