...
describe('explorer REST endpoint counts in Component_Map.md', () => {
    ...
    test('the doc still states a total and a per-namespace breakdown', () => {
        assert.match(doc, /232 REST endpoint patterns across the `\/api` and `\/explorer` namespaces/,
        ...
        assert.match(doc, /14 hand-registered `\/\{COIN\}\/api\/\.\.\.` routes/,
    ...
    test('the hand-registered /api route count matches xchain-explorer source', { skip: !haveExplorer && 'xchain-explorer not present in this checkout' }, () => {
        ...
        assert.equal(routes.length, 14,
            'the explorer hand-registers ' + routes.length + ' /api routes, not the documented 14 :\n  ' +
            routes.join('\n  '));
    });
