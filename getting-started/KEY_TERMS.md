...
**ACTION** — A command embedded in a blockchain transaction that instructs the XChain indexer to perform an operation, such as issuing a token, sending a balance, or placing an order. All XChain operations are expressed as one of 19 named ACTIONs.
...
**xchain-sdk** — The developer SDK for the XChain platform. Provides methods for all 19 actions, 40+ explorer query methods, a batch builder, PSBT generation, and typed error classes.
...
**FILE** — An ACTION that stores a larger data payload on-chain, referenced by hash.

**MESSAGE** — An ACTION that stores a short arbitrary message permanently on the blockchain.
