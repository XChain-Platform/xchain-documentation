...
- **Node.js** 22 (22.x LTS), required to run the `xchain-node` CLI. Node 18 fails on the `mariadb` ESM package (`ERR_REQUIRE_ESM`); Node 24 fails the Node ABI 127 consensus-runtime pin in `xchain-vm`. Node 22 is required.
