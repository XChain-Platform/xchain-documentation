...
**ACTION**: A command embedded in a blockchain transaction that instructs the XChain indexer to perform an operation, such as issuing a token, sending a balance, or placing an order. All XChain operations are expressed as one of 38 named ACTIONs.
...
**xchain-sdk**: The developer SDK for the XChain platform. Provides methods for all 32 developer-invocable actions, 100+ explorer query methods, a batch builder, PSBT generation, and typed error classes.
