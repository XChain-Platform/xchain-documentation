...
const CONFIG_VIEW_PREFIX = String.raw`(?:(?<![A-Za-z0-9_$.])|(?<=(?<![A-Za-z0-9_$.])(?:this\.)?configInfo\.))env`;

const CONFIG_VIEW_READ = new RegExp(
    `${CONFIG_VIEW_PREFIX}\\.([A-Z_][A-Z0-9_]*)(?![A-Za-z0-9_$])` +
    `|${CONFIG_VIEW_PREFIX}\\[\\s*['"]([A-Z_][A-Z0-9_]*)['"]\\s*\\]`, 'g');

// The computed twin of COMPUTED_READ below, through the same view.
const CONFIG_VIEW_COMPUTED_READ = new RegExp(`${CONFIG_VIEW_PREFIX}\\s*\\[\\s*(?!['"])`, 'g');
...
function scanSource(source, opts = {}) {
    ...
    if (opts.configView) {
        CONFIG_VIEW_READ.lastIndex = 0;
        while ((m = CONFIG_VIEW_READ.exec(stripped)) !== null) {
...
function scanComputedReads(source, opts = {}) {
    ...
    if (opts.configView) {
        const lineAt = lineIndex(stripped);
        CONFIG_VIEW_COMPUTED_READ.lastIndex = 0;
        let m;
        while ((m = CONFIG_VIEW_COMPUTED_READ.exec(stripped)) !== null) {
...
module.exports = {
    ...
    CONFIG_VIEW_COMPONENTS, CONFIG_VIEW_READ, CONFIG_VIEW_COMPUTED_READ,
