 ...
 * database_service.js reads three keys through one `process.env[envVar]`), and
...
/*
 * READS THROUGH A CONFIG VIEW. xchain-explorer's src/config.js exports `env`, a
 * live read-through Proxy over process.env, and its code reads configuration as
 * `env.X`, `this.configInfo.env.X` or `configInfo.env['X']` rather than naming
 * process.env. Every such read is an environment read an operator configures,
 * but ENV_READ cannot see it, so moving a read behind the view silently dropped
 * it from coverage: the variable could lose its doc row and this gate stayed
 * green. The view is matched for the components listed here ONLY, because a
 * bare `env` elsewhere in the fleet is whatever object that file named `env`
 * (a child-process environment, a parsed .env file), not process.env.
 *
 * The prefix is a bare `env` (not a member of anything else, so `process.env`
 * stays ENV_READ's alone and is never counted twice) or `configInfo.env`, with
 * or without `this.`. Names are the upper-case environment spelling only, so a
 * method on the object (`env.hasOwnProperty`) is not read as a variable, and an
 * ASSIGNMENT into an object that happens to be called `env` is not a read.
 *
 * A module can also reach the view without holding it: straight off the module
 * (`require('../config.js').env.X`), or through an accessor the file binds to
 * that expression (`const configEnv = () => require('../config.js').env;`, then
 * `configEnv().X`). The accessor defers the require to read time, because
 * config.js loads modules that load these files back, and it is the form the
 * explorer's feature directories use. Its name is whatever the file bound, so
 * it is read from the file rather than assumed: `configEnv().X` in a file with
 * no such binding is somebody else's function and not counted.
 */
const CONFIG_VIEW_COMPONENTS = new Set(['explorer']);

const CONFIG_MODULE = String.raw`require\(\s*['"](?:\.{1,2}\/)+config(?:\.js)?['"]\s*\)`;

const CONFIG_VIEW_ACCESSOR = new RegExp(
    String.raw`(?<![A-Za-z0-9_$.])(?:const|let|var)\s+([A-Za-z_$][A-Za-z0-9_$]*)\s*=\s*\(\s*\)\s*=>\s*` +
    String.raw`${CONFIG_MODULE}\.env(?![A-Za-z0-9_$.\[])`, 'g');

/** Names a source binds as zero-argument accessors returning the config view. */
function configViewAccessors(stripped) {
    const names = new Set();
    CONFIG_VIEW_ACCESSOR.lastIndex = 0;
    let m;
    while ((m = CONFIG_VIEW_ACCESSOR.exec(stripped)) !== null) names.add(m[1]);
    return [...names];
}

/**
 * The named-read and computed-read patterns through the view, for a file that
 * binds `accessors`. The `env` alternatives match at `env` itself; an accessor
 * call matches from its name, which leaves the read's line and the call-shaped
 * default rules unchanged, since both balance the empty `()` they walk over.
 */
function configViewPatterns(accessors = []) {
    const alts = [String.raw`(?:(?<![A-Za-z0-9_$.])|(?<=(?<![A-Za-z0-9_$.])(?:this\.)?configInfo\.)|(?<=${CONFIG_MODULE}\.))env`];
    if (accessors.length) {
        const names = accessors.map((n) => n.replace(/\$/g, '\\$')).join('|');
        alts.push(String.raw`(?<![A-Za-z0-9_$.])(?:${names})\(\s*\)`);
    }
    const prefix = `(?:${alts.join('|')})`;
    return {
        read: new RegExp(
            `${prefix}\\.([A-Z_][A-Z0-9_]*)(?![A-Za-z0-9_$])` +
            `|${prefix}\\[\\s*['"]([A-Z_][A-Z0-9_]*)['"]\\s*\\]`, 'g'),
        // The computed twin of COMPUTED_READ below, through the same view.
        computed: new RegExp(`${prefix}\\s*\\[\\s*(?!['"])`, 'g'),
    };
}

// The accessor-free patterns, exported for callers that scan a fragment.
const { read: CONFIG_VIEW_READ, computed: CONFIG_VIEW_COMPUTED_READ } = configViewPatterns();

/**
 * Is the member expression ending at `end` written to rather than read?
 * `env[key] = value` builds an object; it configures nothing.
 */
function isAssignmentTarget(text, end) {
    let i = end;
...
// The tokens a line may OPEN with and still be finishing the expression the
// previous line started. Deliberately short, and every entry earns its place by
// appearing after a wrapped env read in the fleet: `||` and `??` are the
// fallback itself, `,` is the radix argument on its own line, and `)`/`]`/`}`
// close the call the read sits inside so a numeric coercion's outside fallback
// is still reachable.
//
// The set is a WHITELIST because the failure directions are not symmetric.
// Omitting a token that does continue an expression loses a default, which is
// the silent exemption this scanner already had. Admitting one that does not
// invents a default, and the gate then accuses a doc row that is correct. So
// `.`, `&&`, `?`, `:` and the arithmetic operators stay out: none of them
// introduce a default, and each one is a way to walk into a neighbouring
// expression.
const CONTINUATION_TOKENS = ['||', '??', ',', ')', ']', '}'];

/**
 * Does the source at `i` continue the expression the previous line started?
 *
 * JavaScript has no statement terminator this scanner can rely on: several
 * repos in the fleet are written without semicolons, so a walk that crossed
 * every newline would read the NEXT statement's `|| 'x'` as this read's
 * default. This is the ASI-shaped boundary that stops it, applied only at the
 * read's own bracket depth; inside an open `(`/`[`/`{` a newline is always a
 * continuation and never reaches here.
 */
function continuesExpression(text, i) {
    while (i < text.length && /\s/.test(text[i])) i++;
    return CONTINUATION_TOKENS.some((t) => text.startsWith(t, i));
}
 ...
 *   - a NEWLINE at the read's own depth ends the search unless the next line
 *     opens with a continuation token (see `continuesExpression`);
 ...
 *
 * @param {string} text  the whole comment-stripped source
...
function extractDefault(text, from) {
    ...
    while (i < text.length) {
        ...
        if (ch === '\n' && depth === 0 && !continuesExpression(text, i + 1)) return null;
...
function readOperand(text, i) {
    ...
    // The exponent clause is what stops `|| 1e6` recording `1`. A scan that
    // halts at the `e` reports a default the docs never claimed, so a row
    // correctly saying `1000000` is reported as drift by the checker rather
    // than by the code. Kept NON-capturing: the product scan below reads
    // `more[1]`, and a capturing group there would shift that index.
...
// it opens a regex literal. The one copy: bin/generate-flag-days.js reads
// stripComments from here rather than carrying a twin.
 ...
 * the flag-day generator (bin/generate-flag-days.js), which solved this first
 * and now imports this copy under its old `withoutComments` name.
 *
...
/**
 * Maps a character offset in `text` back to its 1-based line number.
 *
 * Precomputed once per file, because the scans below need a line number per
 * match and counting newlines from the top for each one is quadratic on the
 * larger service files.
 */
 ...
 *
 * SCANS THE WHOLE SOURCE, not one line at a time. The line-split version could
 * not see a fallback that wrapped, so `process.env.X\n    || '/tmp/…'` recorded
 * `default: null`, `comparableDefault` dropped the site, and the drift checks
 * never compared it: the variable could be documented with a completely wrong
 * default and this gate stayed green. Two real reads sat in that blind spot
 * (`XCHAIN_NODE_ENCODER_MAINTENANCE_FILE` in xchain-node, and
 * `CHECKPOINT_FROZEN_TIP_TICKS` in xchain-hub), and the failure direction is a
 * silent pass, which is the one this file exists to stop.
 *
 * `stripComments` blanks rather than deletes, so offsets into the stripped text
 * name the same place in the original and `lineIndex` still reports the real
 * line. The line recorded is the line of the READ, not of the default.
 *
 * With `configView`, reads through the config view (see CONFIG_VIEW_READ) are
 * collected too, merged in source order so a variable's first site is still
 * the first place the file reads it.
 *
 * @param {string} source
 * @param {{configView?: boolean}} [opts]
 * @returns {Map<string, Array<{line:number, default:object|null}>>}
 */
function scanSource(source, opts = {}) {
    const found    = new Map();
    const stripped = stripComments(source);
    const lineAt   = lineIndex(stripped);

    const hits = [];
    ENV_READ.lastIndex = 0;
    let m;
    while ((m = ENV_READ.exec(stripped)) !== null) {
        hits.push({ name: m[1] || m[2], index: m.index, end: m.index + m[0].length });
    }
    if (opts.configView) {
        const { read } = configViewPatterns(configViewAccessors(stripped));
        while ((m = read.exec(stripped)) !== null) {
            const end = m.index + m[0].length;
            if (isAssignmentTarget(stripped, end)) continue;
            hits.push({ name: m[1] || m[2], index: m.index, end });
        }
        hits.sort((a, b) => a.index - b.index);
    }

    for (const h of hits) {
        if (!found.has(h.name)) found.set(h.name, []);
        found.get(h.name).push({
            line: lineAt(h.index),
            default: extractDefault(stripped, h.end),
        });
    }
 ...
 *
 * With `configView`, a computed read through the config view counts as well,
 * unless it is an assignment target. The view is scanned over the whole text,
 * because telling a read from a write needs the closing bracket, which a
 * wrapped key puts on a later line.
 *
 * @param {string} source
 * @param {{configView?: boolean}} [opts]
 * @returns {number[]}
 */
function scanComputedReads(source, opts = {}) {
    const lines    = [];
    const stripped = stripComments(source);
    stripped.split('\n').forEach((code, idx) => {
    ...
    if (opts.configView) {
        const lineAt = lineIndex(stripped);
        const { computed } = configViewPatterns(configViewAccessors(stripped));
        let m;
        while ((m = computed.exec(stripped)) !== null) {
            const close = skipToEnclosingClose(stripped, m.index + m[0].length);
            if (close !== -1 && isAssignmentTarget(stripped, close)) continue;
...
//
// The bound is two characters, not four. A four-character floor meant a short
// variable's row was never recognised as that variable's claim, so its default
// cell stayed in a NEIGHBOUR's asserting set and could launder the neighbour's
// value; the failure direction is a silent pass, which is the one this file
// exists to stop. Single characters stay out on purpose: the doc set has live
// one-letter leading cells (`B`, `T`, `I`, `O`, `H` in the wire field-code
// tables) that are column codes rather than variables, and dropping those rows
// would lose real assertions.
const ROW_SUBJECT = /`([A-Z][A-Z0-9_]{1,})`/;
...
// The end of the matched value, for the UNQUOTED prose form.
//
// `(?![A-Za-z0-9_])` alone let a decimal point through, so a row reading
// "defaults to 30.5 seconds" credited a code default of `30`: the checker
// reported the row correct precisely when it had drifted, which is the silent
// pass this library exists to stop. The backticked and bare-cell forms were
// never exposed, because a closing delimiter has to follow the digits there,
// and that asymmetry is what made the hole invisible.
//
// A `.` or `,` is refused only when a DIGIT follows it, never on its own. A row
// ends its sentence ("Defaults to 30.") and separates a clause ("Defaults to
// 30, and the hub clamps it") far more often than it carries a separator, and
// refusing those would fail correct rows fleet-wide.
const TOKEN_END = '(?![A-Za-z0-9_])(?![.,]\\d)';
...
function assertsDefault(rows, value) {
    ...
    // The delimiters are matched in PAIRS. Interpolating an independent `["']?`
    // on each side accepted `'30000"` and a half-open `` `30000 `` as
    // assertions, which credits a malformed row in the one library the platform
    // trusts to judge whether a doc row is correct. Every branch is
    // non-capturing, so nothing downstream reads a group index out of this.
    const quoted  = `(?:"${v}"|'${v}'|${v})`;
    const wrapped = `(?:\`${quoted}\`|${quoted})`;
    const bareCell  = new RegExp(`^${wrapped}${GLOSS}$`, 'i');
    const asserted  = new RegExp(`\\bdefaults?\\s*(?:to|is|=|:)?\\s*${wrapped}${TOKEN_END}`, 'i');
 ...
 *
 * EVERY value shape must be ASSERTED, because presence alone is VACUOUS.
 * A row documenting a switch names both states by
 ...
 *
 * STRINGS ARE ASSERTED TOO, under the same one rule. The earlier reading gave
 * them a position-free mention, on the ground that a word is not masked by a
 * coincidence the way a digit is. The class that actually ships falsifies it: a
 * row that ENUMERATES the allowed values names the rival value by construction,
 * exactly as a switch row names both states. `SYNC_MODE` is the shipped case,
 * `| \`SYNC_MODE\` | Yes | \`server\` | Operating mode: \`server\` or \`client\` |`
 * against a code default of `server`, where flipping only the Default cell to
 * `client` left the word `server` in the description and the check stayed green.
 * One rule for every value shape, so presence alone is vacuous everywhere.
 */
function defaultDocumented(rows, value) {
    if (value === '0' && rows.some((r) => MEANS_ZERO.test(r))) return true;
    return assertsDefault(rows, value);
}
...
function buildSurvey({ platformRoot, docRoot, serviceReader, docReader, components }) {
    ...
    for (const c of present) {
        ...
        const vars = new Map();
        const computed = [];
        const scanOpts = { configView: CONFIG_VIEW_COMPONENTS.has(c) };
        for (const [rel, source] of files) {
            for (const [name, sites] of scanSource(source, scanOpts)) {
                if (NOT_CONFIGURATION(name)) continue;
                if (!vars.has(name)) vars.set(name, []);
                for (const s of sites) vars.get(name).push({ ...s, file: rel });
            }
            for (const line of scanComputedReads(source, scanOpts)) computed.push({ file: rel, line });
        }
 ...
 * new site (xchain-node/src/services/database_service.js reads three tuning keys
...
const COMPUTED_READ_BASELINE = {
    ...
    decoder: 4, encoder: 4, explorer: 6, hub: 5, indexer: 8, node: 5,
    'regtest-miner': 4, sdk: 4, sync: 7, 'utxo-tracker': 7, vm: 0,
    // sync 8 -> 7 on 2026-09-25, re-measured by the gate against xchain-sync origin/develop a063a71:
    // the L019-6m-sync renames turned src/api.js:72 into a named read, leaving src/coins/index.js:120,
    // :182, :202, :399 and src/config.js:153, :181, :269. Lowered because the blind spot shrank.
    // sync 10 -> 8 on 2026-09-20, re-measured at xchain-sync origin/develop 8fb8e4d5 by the
    // gate itself: two computed reads became named reads since the 2026-09-15 count, so the
    // ratchet drops to the eight remaining sites (src/api.js:72, src/coins/index.js:120, src/coins/index.js:182, src/coins/index.js:202, src/coins/index.js:399, src/config.js:146, src/config.js:171, src/config.js:259). Lowered because the
    // blind spot shrank, the only direction this table may move.
    // regtest-miner 7 -> 4 on 2026-09-18, re-measured against xchain-regtest-miner
    // origin/develop 02a8ac9: the API split moved src/api.js's three computed reads (the
    // REQUIRED_ENV_VARS presence and trim pair at :127 and the parseInt at :134) onto named
    // reads, leaving only the four coin-registry sites (src/coins/index.js:120, :182, :202,
    // :399). Lowered because the blind spot shrank, which is the only direction this ratchet
    // may move. The stale 7 was measured against a local checkout that origin had moved past,
    // so every venue run reddened for every pusher while a local run stayed green: measure this
    // table at the sibling's origin ref, never at whatever a working checkout happens to hold.
    // indexer 7 -> 8 on 2026-09-17, re-derived against the v0.20.0 landing set (xchain-indexer
    // daf70507 plus the edit that reads the witness lever by name): the mirror-admission barrier
    // family (xchain-indexer 8c50c9d4) added bin/verify-mirror-admission-replay-equivalence.js,
    // which landed with four computed reads. Three became literal reads (the side-process now
    // reads XC_MIRROR_ADMISSION_ACTIVATION by name, documented in components/indexer/
    // configuration.md) and one stays: process.env[o.db.passEnv] in explicitDbParams(). Raised
    // deliberately for that one: --db-pass-env NAMES the password variable at run time so the
    // password never reaches argv or the tool's output, so the key cannot be a literal.
    // hub 6 -> 5, indexer 8 -> 7 and sync 11 -> 10 (activation registry W4, row P1/P3): the
    // carrier logic pin's one computed read (bin/lib/carrier_logic_pin.js:119, the
    // XCHAIN_<NAME>_DIR key built from the repo name) became three reads by literal name,
    // so the pin module scans to zero computed sites in all three repos. Lowered because
    // the blind spot shrank: the three variables are now visible to the coverage gate.
    // hub 5 -> 6, indexer 6 -> 8 and sync 10 -> 11 on 2026-09-15, re-measured against the
    // landed tips (xchain-hub d6a81150, xchain-indexer da12b6e0 and xchain-sync 0264a43):
    // the carrier logic pin landed in all three repos (bin/lib/carrier_logic_pin.js),
    // one new computed process.env read apiece (line 119 in each). indexer picks up a
    // second new site: xchain-indexer f3ef3d16 has bin/consensus-identity.js print and
    // pin carrier_logic_digest, reading it from process.env at line 164. Raised because
    // the reads are new, not because the blind spot changed shape.
    // sync 9 -> 10 on 2026-09-15: the code-structure pass's call-time readEnvNow(key) accessor in src/config.js reads the state-tree metric cap, a new computed site.
    // explorer 7 -> 6 and indexer 7 -> 6 at explorer 7b2a28e3 / indexer 29a1b18e: the
    // hub-mirror client split (xchain-indexer c3000819, vendored into explorer)
    // consolidated 11 process.env reads scattered across hub_db_sync.js's methods into
    // one vendored reader, src/hub/hub_db_sync/env.js, whose single readEnvNow(key)
    // is the sole process.env[key] site (line 43) every caller now goes through.
    // Lowered because the count fell, NOT because the unscannable surface shrank: the
    // same keys are read, one hop further away.
    // hub 11 -> 5 at hub f3401e34 (52671173 routed the anchor family's six reads through
    // config.env(), the same one-hop-further shape as below). What remains: the prune
    // tool's one and the four vendored src/coins/index.js sites.
    // hub 33 -> 11 on 2026-09-15, re-derived at the hub stack that follows 21cadb00: the
    // code-structure pass routed the api boot check, the attestation relay's per-coin
    // endpoints, the cross-chain engines' indexer keys, the hub indexer-url and peer
    // roster reads, the spend guard, the CLI child env and the roll-call tunables through
    // hubConfig.env(), which this scanner does not match. The eleven left are the anchor
    // family's six (checkpoint_engine/options.js, publisher/state.js, reorg_handler.js),
    // the prune tool's one and the four vendored src/coins/index.js sites. Lowered because
    // the count fell, NOT because the unscannable surface shrank: the same computed keys
    // are read, one hop further away.
    // hub 35 -> 33 on 2026-09-15, re-derived at xchain-hub 21cadb00: the code-structure
    // pass routed SpendCeiling's two computed reads through the config home's call-time
    // accessor, so src/lib/spend_ceiling.js:60 and :62 now read hubConfig.env()[maxKey]
    // and [windowKey] instead of process.env, which this scanner does not match. The
    // pass's api.js, cross-chain, anchor, attestation and rollcall splits moved the
    // other sites between files without changing the count, which is what counting per
    // component rather than per file is for. Lowered because the count fell, NOT because
    // the unscannable surface shrank: the same <PREFIX>_MAX_PUBLISHES_PER_WINDOW and
    // <PREFIX>_SPEND_WINDOW_MS keys are read, one hop further away.
    // explorer 9 -> 7 on 2026-09-15, re-derived at xchain-explorer 4e73514: the query
    // builder split moved the two envPrefix + '_MS' / '_MAX' reads out of
    // src/db/query_sql.js into src/db/query_sql/query_limits.js, where they read the
    // config view rather than process.env, so the scanner no longer matches them. The
    // seven that remain are the four vendored src/coins/index.js sites, the view's own
    // passthrough in src/config.js and the two in the vendored src/hub/hub_db_sync.js.
    // Lowered because the count fell, NOT because the unscannable surface shrank.
    // indexer 10 -> 7 on 2026-09-15, re-derived at xchain-indexer 4759f9eb: the residue
    // landing routed the bridge transport's three origin-chain reads through the config
    // home (src/consensus/bridge_proof_client/transport.js onto src/config.js). Lowered
    // because the count fell, NOT because the surface shrank: the same per-coin keys are
    // read, one hop further away. The sdk stays 4: xchain-sdk bin/preflight_indexer_root.js
    // reads XCHAIN_ALLOW_NO_INDEXER by its literal name, documented in
    // components/sdk/configuration.md.
    // node 23 -> 5 on 2026-09-14, re-derived at xchain-node d384ef4: the config-home pass
    // (xchain-node 11e4000) declared the computed reads outside the config home as named
    // views in src/config/env_views.js, all bound through one process.env[name] at
    // src/config/index.js:453. Lowered because the count fell, NOT because the surface
    // shrank: the views still build names from a module, coin or network.
    // sync 10 -> 9 on 2026-09-14, re-derived at xchain-sync 0e06951: the same pass
    // (xchain-sync 0c5f1ab) routed the pinned-validator, pool-sizing and shutdown reads
    // through two call-time accessors, src/config.js:155 and :239. Lowered on the same terms.
    // indexer 11 -> 10 on 2026-09-14, re-derived at xchain-indexer 427a0b4: the residue fold
    // (xchain-indexer 57e49dd0) moved src/api.js's REQUIRED_ENV boot check from
    // process.env[key] onto the config home's CONFIG_ENV snapshot, whose every key is a
    // named read in src/config.js. The same commit gave SHUTDOWN_TIMEOUT_MS its named
    // read there, which is what surfaced its row in components/indexer/configuration.md.
    // explorer 7 -> 9 on 2026-09-14, re-derived at xchain-explorer a610975: the scanner
    // now matches reads through config.js's `env` view (CONFIG_VIEW_COMPUTED_READ), so
    // the two envPrefix + '_MS' / '_MAX' reads in src/db/query_sql.js:90 and :201 are
    // counted again. Raised because the blind spot became VISIBLE, not because it grew:
    // no new computed read landed. config.js's own process.env[k] (the view's single
    // passthrough, src/config.js:138) stays in the count. Still counted nowhere, by
    // this or by COMPUTED_READ: a bracket key that OPENS with a quote and then
    // concatenates (`env['UTXO_TRACKER_URL_' + code]`, src/db/readers/entities.js:272,
    // and the two per-coin tip-age keys in src/db/readers/health.js:112 and :128).
    // explorer 8 -> 7 on 2026-09-14: the explorer split db.js into reader modules and
    // routed its env reads through config.js's `env` view. The two computed reads of
    // envPrefix + '_MS' and envPrefix + '_MAX' left db.js and now read
    // this.configInfo.env[...] in src/db/query_sql.js, which this scanner does not
    // match, while the view's own process.env[k] (src/config.js) adds one site that
    // stands for every read through it. Lowered because the count fell, NOT because
    // the unscannable surface shrank: the same keys are read, one hop further away.
    // indexer 8 -> 11 on 2026-09-12: the bridge settle pass landed. bridge_proof_client.js
    // resolves the origin chain's indexer through process.env[coin + '_INDEXER_API_URL'],
    // process.env[coin + '_INDEXER_URL'] and process.env[coin + '_INDEXER_API_KEY']
    // (src/consensus/bridge_proof_client.js:132-134), documented as the <COIN>_INDEXER_* rows in
    // components/indexer/configuration.md. Raised deliberately, for the reason the hub's
    // bridge engine was: an escrow proof is only verifiable against the chain the lock
    // landed on, so the key name is built from the origin coin and cannot be a literal.
    // hub 33 -> 35 on 2026-09-12: the cross-chain bridge engine landed as
    // xchain-hub 9d103c8. CrossChainBridgeEngine.js reads process.env[coin +
    // '_INDEXER_URL'] and process.env[coin + '_INDEXER_API_KEY'] per origin chain
    // (src/CrossChainBridgeEngine.js:158-159), both of which carry rows in
    // components/hub/configuration.md. Raised deliberately: the bridge reads one
    // indexer per origin chain rather than one global endpoint, because a bridged
    // token's source leg is only verifiable against the chain it was locked on,
    // so the key name has to be built from the coin and cannot be a literal.
    // sync 9 -> 10 on 2026-09-11: the persistent replica-gap escalation landed as
    // xchain-sync a4c23f9. ClientSync._numericSetting() reads process.env[key]
    // for REPLICA_GAP_ALERT_SWEEPS and REPLICA_GAP_ALERT_REPEAT_MS, both of which
    // carry rows in components/sync/configuration.md. Raised deliberately: the
    // one helper gives both tunables the same config-first, env-second, never-NaN
    // contract, where an unusable value keeps the documented default instead of
    // making a sweep alert always or never.
    // indexer and explorer 7 -> 8 on 2026-09-11: the hub-mirror stream watermark
    // bound landed as xchain-indexer 74fae9c8 and rode into the explorer with the
    // twin resync xchain-explorer 6cdcb0f. resolveWatermarkStallMs() in
    // src/hub/hub_db_sync.js reads process.env[envKey] for HUB_SYNC_WATERMARK_STALL_S
    // and HUB_SYNC_WATERMARK_STALL_EXIT_S, both of which carry rows in
    // components/indexer/configuration.md and components/explorer/configuration.md.
    // One new site per component, because hub_db_sync.js is a byte twin. Raised
    // deliberately: the indirection is what gives both stall windows the same
    // fallback contract, where an unusable value warns and keeps the documented
    // default instead of throwing, since a bad value here can only mis-time a log
    // line and must never stop a service from booting.
    // indexer 6 -> 7 on 2026-09-06: config.js resolveCoinpayExpiration() reads
    // process.env[envKey] for XCHAIN_COINPAY_EXPIRATION_S, which carries a row in
    // components/indexer/configuration.md. Raised deliberately: the indirection is
    // what lets one resolver enforce the regtest-only rule (ignore-with-warning
    // off regtest, positive-integer check on it) for a consensus input, instead
    // of repeating that guard at the read site.
    // hub 31 -> 33 on 2026-09-03: RollcallRound.resolveTunable() reads
    // process.env[name] twice for the three ROLLCALL_*_BLOCKS tunables, and
    // attest_response_timing.js reads ATTEST_RESPONSE_FORWARD_S_OVERRIDE by its
    // constant. All four names carry rows in components/hub/configuration.md.
    // Raised to match the committed trees, which had landed on origin ahead of
    // the rows; the roll-call indirection is what gives each tunable one NaN guard.
    // node 20 -> 23 on 2026-09-04: ConfigService forwards the three hub-mirror
    // grace windows to the indexer by name (process.env[varName] in one loop), so
    // the operator sets each once on the node and the indexer inherits it. All
    // three carry rows in components/node/configuration.md. Raised deliberately:
    // the indirection is what lets regtest default the trio to 0 while a host
    // value still wins, which a per-name read would have to repeat three times.
    // node 17 -> 20 on 2026-09-03: ConfigService passes the roll-call rail env
    // through to the indexer and hub by name (process.env[varName], read three
    // times in one guard), so the operator sets each variable once on the node.
    // node 16 -> 17 on 2026-09-01, with v0.12.2: ValidatorService.resolveImportedWif()
    // reads process.env[envName] so one function serves both key imports, and the two
    // names it resolves (XCHAIN_NODE_STAKE_WIF, XCHAIN_NODE_DOGE_WIF) each carry a row
    // in components/node/configuration.md. Raised deliberately rather than to clear a
    // gate: the indirection is what lets an import fall back to a hidden prompt when
    // the env is unset, which is the behaviour that keeps a private key off the
    // command line in an interactive install.
    // utxo-tracker 7 -> 8 on 2026-08-20: api.js envInt() centralises validation for the
    // five BULK_SYNC_* knobs, so each is read as process.env[name]. Raised deliberately,
    // not to clear a gate: all five carry rows in components/utxo-tracker/configuration.md,
    // and the indirection buys a NaN guard on knobs a bare parseInt degrades silently.
    // utxo-tracker 8 -> 7 on 2026-09-12: the numeric operator-override validation
    // replaced one computed read with a named one, so the ratchet drops to match.
};
...
module.exports = {
    ...
    CONFIG_VIEW_COMPONENTS, CONFIG_VIEW_READ, CONFIG_VIEW_COMPUTED_READ, configViewAccessors,
