...
- **Cross-chain swaps** (`GET_COIN` ≠ the posting chain) escrow the GIVE side locally and are
  ...
  See [Cross-Chain DEX](../cross-chain-dex.md).
...
- Use `^` (caret) as prefix when passing an `ADDRESS_ID` for `GET_ADDRESS` (^57 = `ADDRESS_ID` 57); see [Index ID References](../index-id-references.md)
