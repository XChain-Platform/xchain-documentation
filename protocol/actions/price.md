...
### Version 0: Validator COIN/FIAT Price Rounds (batched)
...
- A distinct tag is required, not optional. The equivocation judge reads a signed content's `round` field to compare two things a pubkey signed at one anchor; a v2 canonical has no `round` field (it has `first_round`/`last_round` instead), and reusing `XORACLE` for it would either break that read or manufacture a false equivocation between an honest v0 round and an honest v0 batch signed by the same validator at the same anchor. The window (`first_round|last_round`) inside the round id also keeps two different, equally honest batch splits at one anchor from colliding on the same equivocation key
...
- At and above the gate, a well-formed v2 action is valid on every node; a one-sided deploy (some nodes carrying the parser, some not) would fork the fleet on the first v0 batch, so the parser, the gate, the equivocation tag and the straddle check all ship together, ahead of any node actually publishing v2
