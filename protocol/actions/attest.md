...
#### Widening: how the served set can grow beyond REDUNDANCY

Independent of the two rules above, the set actually served can widen beyond the `REDUNDANCY` validators ranked above. At and above `ATTEST_RESPONSIBLE_WIDENING_ACTIVATION`, up to `ATTEST_RESPONSIBLE_WIDENING.maxSlots` (2) further validators are admitted as the request's serviceable window elapses, one slot per equal segment of that window, starting `ATTEST_RESPONSIBLE_WIDENING.confirmations` (3) blocks after the request. At and above `ATTEST_ZERO_CONF_ACTIVATION`, keyed on the request's own block, the ladder runs a second stage instead, `ATTEST_RESPONSIBLE_WIDENING_V2`: it starts at the request's own block (`startOffset` 0, matching zero-confirmation service, below) and grants one extra slot immediately (`headroom` 1) rather than waiting for the first segment to elapse, so a set holding one member that can never sign still finalizes inside the first segment with no clock involved at all; `maxSlots` stays 2, so the served pool can still reach `REDUNDANCY + 3` members in the worst case. A request whose deadline falls at or before the ladder's own start is granted headroom outright rather than dividing a non-positive window.

Widening only ever changes who may additionally serve and earn a share of the fee (see Fee flow, below); it never changes the set a request is admitted against or the set charged for a missed round in per-validator accountability, which stays the unwidened, `REDUNDANCY`-ranked set throughout.
...
### Zero-confirmation service, above `ATTEST_ZERO_CONF_ACTIVATION`

Above a network's `ATTEST_ZERO_CONF_ACTIVATION` height, keyed on the request's own block, three more changes apply together, because a mixed fleet must agree on the leader slot, the responsible set and the applied row identically for the same request:

- The responsible set is served the moment the request mines, at the tip block, rather than after a confirmation wait. The `attestation` capability snapshot a request is served against is still resolved the same reorg-buried number of blocks below the request as always; the wait removed here only ever protected a hub's own provider spend against a request that later reorgs, never the capability snapshot itself.
- The widening ladder (see Responsible-set selection, above) runs its second stage from the request's own block, so a set holding one member that can never sign can still finalize immediately.
- The mirror applier falls through an inert first candidate response to the next admissible one, described below, instead of leaving the request stuck retrying the same inert candidate until its deadline.

`ATTEST_ZERO_CONF_ACTIVATION` is non-null for a network only where the response-mirror height and the widening ladder's own activation height are both non-null and no higher than it, so a network is never served at zero confirmations without the mirror already delivering answers and the ladder's headroom already able to absorb a stalled member.

More than one round can finalize for the same request (for instance a round that finalizes twice under two leader slots), so more than one agreed answer can reach the mirror for it. Below `ATTEST_ZERO_CONF_ACTIVATION` the indexer tries only the earliest candidate, by signed `effective_time`, tie-broken by the candidate's own response hash; if that candidate can no longer be admitted (its signers have since fallen out of the responsible set, for instance) the request stays pending until its deadline, because the same candidate is re-selected and re-refused at every later block. At and above the height, keyed on the request's own block, the indexer instead orders every candidate for the request the same way on every node and tries them in order, binding the first one it can admit. Exactly one candidate ever binds for a request; the rest remain visible in the mirror table for audit.
...
At and above `ATTEST_ZERO_CONF_ACTIVATION`, keyed on the request's own block, a `fulfilled` settle changes once more. Because the widening ladder now grants headroom from the request's own block (see Responsible-set selection, above), the recomputed responsible set can include a member the round never actually needed and that never signed. So above the height the escrow instead splits equally among the pubkeys that verifiably signed the accepted response, sorted by pubkey before paying, rather than the recomputed responsible set; each signer's `validator_rewards` row and the `bcmulfloor` rounding are otherwise unchanged. `errored` and `expired` refunds are untouched by this change: a request that never fulfills has no signer set to pay, so it keeps refunding `FEE_PAYER` exactly as above.
