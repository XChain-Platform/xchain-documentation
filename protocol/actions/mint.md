...
- If the token binds a `mint`-class controller (or the catch-all `all`), the contract's `guard` runs before the mint settles and may `revert` it; `SOURCE` pays the bounded guard gas. See [Controller-Bound Tokens](../controller-bound-tokens.md).
