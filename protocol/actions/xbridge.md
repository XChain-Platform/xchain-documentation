...
## Rules
- Below `XCHAIN_BRIDGE_ACTIVATION` for v0 to v2 or `TOKEN_BRIDGE_ACTIVATION` for v3 to v5 on this chain (both maps are keyed `'<COIN>:<network>'`, falling back to the bare network key), every version returns `invalid: XBRIDGE before activation`. The height compared is always the block index of the chain being parsed, never a transfer's snapshot block.
