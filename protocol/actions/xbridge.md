...
## Rules
- Below `XCHAIN_BRIDGE_ACTIVATION` for this chain (v0 to v2; the map is keyed `'<COIN>:<network>'`, falling back to the bare network key) or `TOKEN_BRIDGE_ACTIVATION` for this network (v3 to v5), every version returns `invalid: XBRIDGE before activation`. The height compared is always the block index of the chain being parsed, never a transfer's snapshot block.
