 ...
 **********************************************************************
 *
 * The documentation repo's activation registry: every flag-day table the
 * twinned reference-impl carriers beside this directory read, as (key,
 * value) rows keyed '<stem>.<EXPORT>', the spelling the fleet's rules digest
 * and signed GATES field already use. The carriers keep their predicates and
 * read their tables from here, so a table lives in ONE place per repo and a
 * moved carrier can no longer read as "not yet active".
 *
 ...
 * this process's environment as it stands at that moment. This repo carries
 * no runtime service and no config layer (protocol/reference-impl is a
 * conformance source of record, not a deployed carrier), so the overlay
 ...
 *
 * DOCUMENTATION-ONLY GATES: none. Every twinned carrier's table is in the
 * SHARED block, so this repo registers no rows of its own.
 *
 ********************************************************************/
