...
You don't have to start from a blank file. The `xchain-contracts` library ships 14 audited, deploy-ready templates and 5 reusable patterns (access control, pausable, safe-transfer, input validation, state machines). The flow is **scaffold → customize → lint → deploy**.

The templates are escrow, vesting, crowdsale, amm, treasury, cardDispenser, urlOracle, priceBet, priceBetTimed, stableVault, escrowDelivery, englishAuction, dutchAuction, and counterpartyBridge. Each ships with a walkthrough README, a test suite that runs the real contract through the XChain VM, and an explicit "attacks we considered" section. Start with escrow: it explains the custody model the rest build on.
